 ...
 *     - a .tgz archive from npm: "https://registry.npmjs.org/native-graphql-scripts/-/native-graphql-scripts-0.20.0.tgz"
 *     - a package from `tasks/clean_pack.sh`: "/home/adam/create-native-graphql-app/native-graphql-scripts-0.22.0.tgz"
 */
...
if (commands.length === 0) {
  if (argv.version) {
    ...
    console.log(`create-native-graphql-app version: ${version}`);
    process.exit();
  }
  console.error('Usage: create-native-graphql-app <project-directory> [--verbose]');
  process.exit(1);
}
...
async function createApp(name: string, verbose: boolean, version: ?string): Promise<void> {
  ...
  console.log('Installing native-graphql-scripts...');
  console.log();
...
function getInstallPackage(version: ?string): string {
  let packageToInstall = 'native-graphql-scripts';
...
async function checkNodeVersion(packageName: string): Promise<void> {
  ...
  if (!semver.satisfies(process.version, packageJson.engines.node)) {
    console.error(
      chalk.red(
        'You are currently running Node %s but create-native-graphql-app requires %s.' +
