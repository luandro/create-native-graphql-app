...
**WARNING**: Just don't release `create-native-graphql-app`. Just don't do it.

The rest of these instructions are for releasing `native-graphql-scripts`.
...
After upgrading the [expo-sdk](https://github.com/exponent/exponent-sdk) version (which transitively updates the `react-native` version), ensure that the [.flowconfig](native-graphql-scripts/template/.flowconfig) template is up to date.
...
3. If there are changes, land diff to CRNA [.flowconfig](native-graphql-scripts/template/.flowconfig) template
4. To test, follow instructions in [README/Adding Flow](native-graphql-scripts/template/README.md#adding-flow) on a freshly generated project, and do `npm run flow` to ensure the process exits without error.
