...
We ${chalk.italic('strongly')} recommend that you read this document before you proceed:
  ${chalk.cyan('https://github.com/luandro/create-native-graphql-app/blob/master/EJECTING.md')}
    ...
    const questions = [
      {
        type: 'list',
        name: 'ejectMethod',
        message: 'How would you like to eject from create-native-graphql-app?',
...
from \`babel-preset-expo\` to \`babel-preset-react-native-stage-0/decorator-support\`.`
      ...
      delete pkgJson.devDependencies['native-graphql-scripts'];
